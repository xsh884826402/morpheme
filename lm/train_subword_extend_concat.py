...
from word2vec import Word2Vec_subword_extend_concat,train_epoch
...
class Config():
    def __init__(self):
        ...
        self.if_test = False
        self.if_extend = True
        self.model_path = '../data/lm_model_subword_extend_concat/'
...
def main():
    ...
    config = Config()
    model = Word2Vec_subword_extend_concat(config)

    gpu_options =tf.GPUOptions(per_process_gpu_memory_fraction=0.5, allow_growth=True) ##每个gpu占用0.5的显存
    ...
    with tf.Session(config=tf_config) as sess:
        if config.if_train:
            outfile = os.path.join(os.path.dirname(config.original_file), 'training_data_' + str(config.vocabulary_size) + '_winsize' + str(
                config.skip_window) + '_subword_extend_concat')
