# 每次更新 显卡，self.model_path,outfile
# 注意每次更新使用的subword——dict
...
class Config():
    def __init__(self):
        ...
        self.if_test = False
        self.if_extend = True
        self.model_path = '../data/lm_model_subword_extend_using_bpe_2/'
    ...
    with tf.Session(config=tf_config) as sess:
        if config.if_train:
            outfile = os.path.join(os.path.dirname(config.original_file), 'training_data_' + str(config.vocabulary_size) + '_winsize' + str(
                config.skip_window) + '_subword_extend_using_bpe_2')

            subword_dict_path = '../data/bpe_subword.txt'
